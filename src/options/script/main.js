...
async function loadFromStorage() {
    ...
    // Load resetTime and taskTime from storage
    document.settings.resetTime.value = data.resetTime || '08:00'; // Default to 8:00 AM
    document.settings.taskTime.value = data.taskTime || '21:00'; // Default to 9:00 PM
}
...
function saveSettings() {
    browser.storage.sync.set({
        ...
        resetTime: document.settings.resetTime.value,
        taskTime: document.settings.taskTime.value
    });
}
...
function tasksPage() {
    ...
    const now = new Date();
    const taskTime = document.settings.taskTime.value || '21:00'; // Default to 9:00 PM
    const taskTimeDate = new Date(now);
    taskTimeDate.setHours(parseInt(taskTime.split(':')[0]), parseInt(taskTime.split(':')[1]), 0, 0);

    // If it's already past 9:00 PM, run createTasksPage immediately
    if (taskTimeDate < now) {
        browser.runtime.sendMessage({ command: 'CREATE_TASKS_PAGE' });
    } else {
        window.alert("The tasks page will be available after " + taskTime + ". Come back then!");
    }
}
...
document.getElementById('create-tasks-button').addEventListener('click', tasksPage);
